      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          const int actuatorlatency = 100;

          // Actuator latency in seconds.
          const double latency = actuatorlatency / 1000;

          // Initial state.
          const double x0 = 0;
          const double y0 = 0;
          const double psi0 = 0;
          const double cte0 = coeffs[0] - 0;
          const double epsi0 = 0 - atan(coeffs[1]);

          // State after latency.
          double x_latency = x0 + ( v * cos(psi0) * latency );
          double y_latency = y0 + ( v * sin(psi0) * latency );
          double psi_latency = psi0 - ( v * delta * latency / 2.67);
          double v_latency = v + a * latency;
          double cte_latency = cte0 + ( v * sin(epsi0) * latency );
          double epsi_latency = epsi0 - ( v * atan(coeffs[1]) * latency / 2.67 );
          ...
          // SUBMITTING.
          this_thread::sleep_for(chrono::milliseconds(actuatorlatency));
